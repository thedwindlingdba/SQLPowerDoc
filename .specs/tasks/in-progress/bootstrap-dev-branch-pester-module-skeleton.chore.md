...
### Step 1: Create and push `dev` branch [DONE]

**Dependencies:** None  
**Success criteria:**

- [X] `git checkout -b dev` from default branch (`master` or `main`).
- [X] Branch exists locally; `git push -u origin dev` when remote available.
...
### Step 2: Add `tests/Run-AllTests.ps1` Pester harness [DONE]

**Dependencies:** 1  
**Success criteria:**

- [X] Script sets `$ErrorActionPreference = 'Stop'` where appropriate.
- [X] Invokes `Invoke-Pester` with `Run.Path` = `tests`, Pester 5+, exit non-zero on failure (`-CI` or `Run.Exit`).
- [X] Resolves repo root via `$PSScriptRoot`.
...
### Step 3: Document Pester installation [DONE]

**Dependencies:** 1 (parallel with 2)  
**Success criteria:**

- [X] `Install-Module Pester -MinimumVersion 5.0.0` (or scoped variant) documented.
- [X] Minimum PowerShell version stated (target **7+** aligned with modernization plan).
...
### Step 4: RED — failing LogHelper import test [DONE]

**Dependencies:** 2  
**Success criteria:**

- [X] `tests/LogHelper.test.ps1` uses `BeforeAll` / `Import-Module` with path to `Modules/LogHelper/LogHelper.psd1` (GREEN path).
- [X] Controlled failure for import is validated by the Step 5b vacuous-test recipe (rename manifest → non-zero exit).
...
### Step 5: GREEN — make import test pass [DONE]

**Dependencies:** 4  
**Success criteria:**

- [X] `Invoke-Pester` / `Run-AllTests.ps1` exits 0 with LogHelper import test passing.
- [X] Prefer fixing harness (`PSModulePath` prepend or `-File` resolution) over large LogHelper rewrites.
...
### Step 5b: Regression check for AC4 (vacuous-test guard) [DONE]

**Dependencies:** 5  
**Success criteria:**

- [X] Create **`tests/README.md`** if missing. Add subsection **Verify tests are not vacuous** with a **repeatable** recipe: temporarily rename `Modules/LogHelper/LogHelper.psd1` to `LogHelper.psd1.bak`, run `tests/Run-AllTests.ps1`, confirm **non-zero exit**, restore the manifest. Warn: only on `dev`, clean working tree, do not commit the rename.
...
### Step 6: Smoke test `Write-Log` to temp file [DONE]

**Dependencies:** 5b  
**Success criteria:**

- [X] Test calls `Set-LogFile`, `Set-LoggingPreference`, `Write-Log`; asserts file contains message.
- [X] Handles LogHelper queue flush (unload module or documented wait).
...
### Step 7: Optional LogHelper Public/Private split — default DEFER [DONE]

**Dependencies:** 6  
**Success criteria:**

- [X] Record **defer** decision in **`tests/README.md`** only (subsection **Optional module layout**); **or** execute minimal split if Step 5 required it.
...
### Step 8: Final documentation sync [DONE]

**Dependencies:** 3, 6, 5b, (7 if executed)  
**Success criteria:**

- [X] README: branch name **`dev`**, **Running tests** with exact command; link to `tests/README.md` for vacuous-test recipe.
- [X] `requirements.md`: Pester + PowerShell minimum.
...
- [X] `dev` branch created and documented.
- [X] `tests/Run-AllTests.ps1` runs Pester 5 with correct exit codes.
- [X] LogHelper import test passes; fails when import intentionally broken.
- [X] `Write-Log` smoke test passes with file assertion.
- [X] README + requirements updated.
- [X] `Invoke-Pester` / `Run-AllTests.ps1` executed successfully in this environment after implementation.
...
**Plan judge score (orchestrator):** 4.6/5.0 (iteration 2 PASS)

**Implement verification (orchestrator):** Self-check ≥ 4.5/5.0 — Pester suite green (`Run-AllTests.ps1` exit 0); harness uses Pester 5 + `Run.Exit`; docs and vacuous-test recipe present; `Get-SqlServerVersionName` covered via fixture pending full module PS7 import.
